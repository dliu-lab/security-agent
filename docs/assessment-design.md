...
**Aligned baseline:** HLD v0.9 — reviewed 21 September 2026.
...
- Use Strands Agents SDK for hosted agent orchestration, with AgentCore SDK Runtime integration and explicit loading of the two trusted scanner skills.
- Authenticate hosted user and approved machine clients with corporate JWT; authorise each target, operation and report using verified identity.
...
| Assessment scope | `source_only` or `deployment`; source-only scans do not require a deployment identity |
| Target environment and instance | For deployment scope, `environment_ref` or explicit `unspecified`, `instance_ref` for the deployment/installation, and declared/verified association; independent of scanner execution location |
| Configuration set | Evidence references/digest, intended environment/instance, source and capture time, precedence, unresolved values and declared/observed provenance |
...
Freeze the inventory after bounded resolution. Reusable repository subject identity includes snapshot and relative package path; installed subject identity includes capture, host, configuration scope and registration. Target evaluation identity additionally includes `assessment_scope` and, for deployment scope, `environment_ref` and deployment/installation instance reference. If the environment is unknown, record `unspecified` and its limitation; never infer production from a branch, path or endpoint display name. Keep duplicate or shadowed registrations linked, not collapsed by display name. Parse identical package bytes once, but evaluate each environment/instance's configuration, permissions and associated endpoints separately. Preserve source-to-installed-package and source/installed-package-to-deployment correspondence independently as `verified`, `unverified` or `mismatched`, with supporting evidence. Package/version names alone cannot verify correspondence.

**Deployment configuration inputs.** Accept approved configuration files, overlays and captured settings through the existing collectors/evidence contract, scoped to a target environment and instance. Preserve input precedence, declared versus observed provenance, capture time, unresolved substitutions and conflicting settings; do not execute templates or credential helpers to resolve them. Bind the selected exported/captured artifacts and their configuration-set digest into the manifest, coverage and result-cache inputs. Source defaults are evidence about the repository; a deployment conclusion requires evidence of the applicable configuration and its correspondence. Missing, redacted, conflicting or unverified required configuration leaves the affected deployment checks unknown. Source-only assessment remains valid without these inputs. Supplying configuration grants no cloud API access or additional endpoint permission.
...
The proposed bundle manifest contains schema/collector/host-adapter versions; capture time and location; repository revision or installation-capture ID; target/registration identities; assessment scope, environment/instance association and provenance; file paths relative to approved roots and exported-file digests; configuration-set references/digest, precedence and unresolved values; declared/effective-state evidence and limitations; endpoint observations with access context and timestamps where authorised; exclusions/redactions; and collection errors/limits. Use portable root identifiers rather than exposing unnecessary user home paths. Validate archive paths, symlinks, expansion ratio, file count/size and total processing bounds before accepting evidence. Freeze validated contents under an immutable artifact reference. Finding confidence is evaluated separately through the two fields in section 7, not trusted from the bundle.
...
The following are proposed selector fragments, not implemented commands. Use the shared mode/version/idempotency envelope. Hosted `start_assessment` accepts exactly one of `repository_scope` or `targets`; direct local submission additionally accepts `installation_scope`. `input_kind` is recorded in the resolved inventory, not used as a substitute for `target_type`. Require an explicit `assessment_scope`: repository-only scans use `source_only`; endpoint and installation-context assessments use `deployment` with an environment reference or explicit `unspecified` and a resolved instance reference. Associate configuration evidence explicitly. For multiple environments, create separate target evaluations over the shared subject rather than one combined verdict.

| Entry point | Submission selector | Resolution |
| --- | --- | --- |
| Skills repository | `repository_scope`: `assessment_scope: source_only`, snapshot evidence reference, `target_types: [skill]`, approved paths and inventory limits | One batch with a stable source-only target ID per discovered skill |
| MCP source repository | `repository_scope`: `assessment_scope: source_only`, snapshot evidence reference, `target_types: [mcp]`, package paths and inventory limits | One or more explicitly resolved MCP package targets; ambiguous candidates remain visible |
| MCP endpoint | `targets`: `target_type: mcp`, `assessment_scope: deployment`, `environment_ref` or `unspecified`, `instance_ref`, registered `endpoint_ref` and approved discovery grant/credential context | One MCP target with bounded endpoint observations and associated configuration evidence where supplied; source is optional |
| Installed skill/MCP, local | `installation_scope`: `assessment_scope: deployment`, `environment_ref` or `unspecified`, supported host, project context, selected scopes/registrations, permitted read roots and limits | Resolve an `instance_ref` per installation with configuration evidence, then run skill/MCP checks |
| Installed skill/MCP, hosted | `targets`: resolved skill/MCP IDs, `assessment_scope: deployment`, `environment_ref` or `unspecified`, `instance_ref` and immutable exported-bundle `evidence_refs` | Hosted static assessment of supplied installation/configuration evidence; additional endpoint access requires a separate grant |
...
Collection and parsing dominate many batches, so snapshot a repository once and share an immutable file index. Reuse common dependency/plugin observations, but group findings only where per-target outcomes, severity and confidence remain equivalent. Schedule target checks with bounded pools and keep environment/instance-specific configuration checks separate. Policy-result reuse additionally binds scope/environment/instance, configuration-set digest/provenance, catalogue identity/digest, mapping/policy/rule versions, approved scanner-output provenance/freshness where supplied, access and coverage. Changed inputs require reevaluation; resume preserves the original binding or starts a new assessment. Endpoint/auth-context observations need separate freshness handling. Twenty source-only skills produce twenty target records within a batch, not twenty clones, model agents or Runtime sessions; multiple deployment contexts add evaluations without requiring repeated clones. Deep mode sends bounded finding/evidence groups rather than entire repositories to Bedrock.
...
Implement the Runtime HTTP contract through the AgentCore SDK: `/invocations` for requests and `/ping` for health. Corporate JWT is the selected inbound authentication mode; clients use authenticated HTTPS with a token from the approved corporate identity integration. Configure and verify issuer/discovery settings, audiences, scopes and user/machine principal mappings. Authorise every assessment, target, and report operation. Derive ownership from verified identity or a trusted identity propagation layer, never request-body owner fields or a session ID. Verify identity propagation as part of deployment; transport authentication alone does not provide per-report ownership. See [HTTP contract](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-http-protocol-contract.html) and [inbound authentication](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-oauth.html).
...
A plugin groups versioned skills and supporting resources; it is not another deployed agent or container. Strands Agents SDK is the selected hosted framework. Configure its `AgentSkills` integration to load only the two trusted skill directories, while the application supplies bounded resource-access and engine tools. A Claude `.claude-plugin/plugin.json` manifest is not automatically interpreted by AgentCore or by Strands. Pin and validate Strands and AgentCore SDK compatibility during implementation. See [Strands skills](https://strandsagents.com/docs/user-guide/concepts/plugins/skills/).

In a local compatible coding assistant, use the canonical scanner plugin for direct execution, or select the separate remote client for hosted execution. The two roles must be explicit; installing the local scanner does not require using the hosted API. The separate scanner-plugin repository is canonical for skill source and the marketplace distributes its approved releases. The Security Agent repository owns the executable engine and emits both a local package and hosted image. Share versioned API/tool contracts and avoid duplicate engine implementations or a second editable copy of scanner skills. The local assistant adapter remains a separate supported interface from the hosted Strands adapter. Pin host adapters and plugin/engine versions, and verify that they do not activate unapproved plugin components. If a Claude plugin format is used, its manifest and namespacing serve compatible local hosts; the Strands adapter explicitly registers only the two skill directories. See [Claude Code skills](https://code.claude.com/docs/en/skills), [plugin packaging](https://code.claude.com/docs/en/plugins), and the [Agent Skills format](https://agentskills.io/specification).
...
### Internal catalogue and mapping validation

CISS files are internal corporate control material and remain outside this Git repository, plugin bundles and public research. Review feedback reports an existing OSCAL catalogue resolved through an internal locator. Confirm the authorised location, access and actual schema before building the adapter; the locator and catalogue contents need not be committed. Import exact control and clause identifiers and their relationships. Catalogue import supplies authoritative metadata, not automatic rule mappings, applicability decisions or policy thresholds. Use synthetic catalogues only for isolated tests and clearly labelled prototypes.

Pin `catalog.uuid`, `metadata.version`, `metadata.oscal-version` and a verified content digest, plus independent mapping/policy/rule artifact versions and digests. Document version and OSCAL schema version are different. OSCAL defines control status through properties, including `status=withdrawn` in its namespace; it does not use a universal top-level withdrawal boolean. Validate the actual CISS encoding for clauses and any extension namespaces rather than assuming control-level semantics apply unchanged. [NIST OSCAL catalogue reference](https://pages.nist.gov/OSCAL-Reference/models/v1.2.3/catalog/json-reference/).

At load, validate referenced IDs, parent/child applicability and control/clause status against the pinned artifact. Reject withdrawn controls or clauses as active mandatory obligations, including a withdrawn clause inside an otherwise active control. Missing IDs, digest mismatch or uninterpretable status for a required mapping are configuration errors and block the corporate verdict; technical findings remain reportable. Do not silently delete a requirement or map it to a suggested replacement. Preserve withdrawn, deprecated and superseded distinctions under reviewed policy. Historical reports retain their original catalogue binding; a catalogue update triggers mapping review and a new assessment rather than rewriting history.

### Scanner governance and dependency evidence

Resolve Security Agent's tool classification, approval scope and configuration-review obligations with the corporate policy owner in implementation steps 1–2. The particular approval requirement and named-product restrictions in review feedback have not been verified against internal CISS; this design does not assert those details as established policy. Document the decision and obtain any required approval before operational use. Limiting dependency work to evidence collection does not automatically exempt the wider scanner.

V1 collects dependency inventory, declared versions, pinning, lockfiles and package/image provenance. It does not match packages against a vulnerability feed to determine CVE applicability or independently assign vulnerability severity. An approved, bounded adapter may ingest supplied results from an approved scanner where in scope, retaining producer/version, subject identity, scan time, data version, evidence digest and original severity. Report these as attributed external determinations, separately from engine findings, and apply reviewed subject/freshness policy before using them in corporate evaluation. Absent, stale or mismatched results cannot establish vulnerability coverage. This requires no new external API or product dependency. Broader vulnerability detection would require an explicit scope and approval decision.
...
| Supply chain | Dependency inventory, pinning, lockfiles and package/image provenance; separately attributed approved scanner outputs where supplied, with subject/freshness validation; no engine CVE determination |
...
Select checks using the target’s declared and observed protocol version and features. Do not assume a single initialization, discovery, or session model across revisions. Maintain a supported SDK/revision matrix and conformance fixtures for each revision; unsupported or inconsistent revisions produce explicit limitations. Review the [MCP revision changelog](https://modelcontextprotocol.io/specification/2026-07-28/changelog) when implementing collectors.
...
| Dependencies and provenance | Dependency inventory, pinning, lockfiles, package identity and unverifiable binaries; separately attributed approved scanner outputs where supplied; no engine CVE determination |
...
Every rule should declare its stable identifier and version, applicability, required evidence, evaluation method, versioned severity policy with permitted factors/defaults/ranges and rationale, separate match/conclusion confidence criteria, reviewed mappings, limitations, remediation template and verification procedure. Evaluate severity for each instance from its own evidence and target context; a rule-level default is not a substitute for that evaluation. Do not copy severity triggered by one code location onto another. Evidence that disproves the predicate changes the check outcome rather than merely reducing severity. Use an explicit result for missing evidence.

`match_confidence` states certainty that the evidence meets the rule predicate; `conclusion_confidence` states certainty of the security interpretation in the assessed context. Each contains a value from the reviewed vocabulary, its basis and limitations. An exact pattern match may have high match confidence and low conclusion confidence when deployment context is missing. Neither field is a calibrated probability or a substitute for evidence coverage. Models cannot amend them.
...
- Repository revision or snapshot identity, assessment scope, environment/instance association, configuration-set references/digest, and source-to-deployment correspondence where applicable.
- Observed, declared, or inferred evidence classification.
- Corporate mappings with catalogue UUID/version/schema/content digest, mapping/policy versions and resolved control/clause status; external categories separately.
- Evaluated instance severity, severity-policy version, contributing factors and evidence-specific rationale.
- `match_confidence` and `conclusion_confidence`, each with value, basis and limitations; remediation and verification steps.
...
GHA uses an approved corporate machine-JWT flow for the Security Agent API. Do not assume a GitHub OIDC token or AWS credentials are accepted directly by that endpoint. GitHub OIDC may separately supply AWS role credentials for authorised AWS operations. Keep assessment API, AWS and GitHub write credentials separate. [OIDC with AWS](https://docs.github.com/en/actions/how-tos/secure-your-work/security-harden-deployments/oidc-in-aws).
...
1. Resolve scanner classification/approval requirements early, validate the internal CISS adapter, and define environment-scoped repository/endpoint/installation selectors, portable bundles, instance severity/confidence, coverage and status-aware mapping contracts. Keep synthetic controls confined to test fixtures/prototypes.
...
Use the same source in two environments with different overlays: preserve separate results and shared parsing, and prevent a secure repository default from masking an insecure deployment override. Missing/conflicting/unresolved required configuration must produce unknown coverage; source-only scans must finish without fabricated environment evidence. Configuration changes invalidate policy-result reuse and cannot alter an assessment during resume. Test two instances of one rule with different supported severities, and high match confidence with low conclusion confidence. Exercise active/withdrawn control and clause mappings, unknown IDs/status encodings, digest mismatch and reviewed replacements; mapping failures preserve findings but block corporate verdicts. Supplied scanner results retain attribution and cannot silently become engine CVE determinations. Use revision-specific MCP conformance fixtures.
...
Decisions still needed: authorised CISS location/access, actual OSCAL schema and clause-status semantics, reviewed mappings and catalogue pin; scanner classification/approval and configuration-review obligations; environment/instance identifiers and configuration precedence; supported MCP revisions/transports and skill/plugin dialects; discovery allowlists and credentials; instance severity, both confidence vocabularies and mandatory-unknown policy; local/hosted evidence retention/redaction; any approved supplied scanner-output format and freshness policy; approved Bedrock profile/model/regions; approved Strands/AgentCore SDK versions and local host adapters; plugin/local engine/optional client distribution and compatibility; corporate JWT provider configuration, user/machine token flows and verified identity propagation; Aurora region/version, provisioned or Serverless v2 capacity, Data API availability/limits and secret management; local filesystem/sandbox and cloud isolation requirements; measured batch/runtime budgets; whether automatic recovery is required beyond explicit resume; and reassessment cadence.
