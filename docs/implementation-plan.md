...
**Design baseline:** [HLD v0.9](high-level-design.md) and [assessment design](assessment-design.md), including the four intake paths and optional GHA remediation PR. The HLD remains the architecture authority; this document defines delivery order and acceptance gates.
...
| 2 | Versioned contracts, internal catalogue adapter and fixture corpus | 1 | Engine, security and API engineering |
...
Steps 5, 6 and 8 can progress independently once their interfaces are stable. Step 7 can start with source assessments while the other collectors are built. Platform feasibility and corporate scanner-classification work start in step 1; production configuration is completed before step 9. The internal CISS catalogue is reported to exist in machine-readable OSCAL form; its location, access permissions and specific encoding still require verification. Pending cloud or catalogue access need not block an isolated synthetic local prototype, but the corresponding real-input acceptance gates remain open.
...
Locate the authorised internal CISS source and confirm its OSCAL schema, metadata, control/clause identifiers, status representation and permitted handling with its owner. Keep catalogue content and private policy details out of this repository. Resolve Security Agent's corporate scanning-tool classification, applicable approval and configuration-review requirements in steps 1–2, before committing to real scanning capabilities. The review's specific approval-policy claim is an input to verify against internal policy, not an established exemption or approval. Plan v1 dependency output as inventory, pinning, lockfile and provenance evidence without engine CVE determination; if vulnerability results are required, define bounded ingestion of supplied results from an approved scanner. This adds no external runtime API and does not exempt Security Agent from any applicable approval.
...
**Done when:** package CI builds an empty installable application; source ownership and compatibility strategy are documented; catalogue access/interpretation and scanner classification have named owners and explicit decisions or blockers. Any required approval gates precede real scanning/pilot use. Feasibility results identify which deployment inputs are available and which remain blockers. No cloud capability is declared supported solely because a mock passed.
...
- Assessment scope: `source_only` or `deployment`; deployment scope carries `environment_ref` or explicit `unspecified` and an `instance_ref` with association provenance. Deployment/installation identity is separate from source/package identity; source-only scans require neither deployment field.
- Evidence set: source, installed bytes, configuration and endpoint observations, with provenance and correspondence. Bind configuration evidence references/digests, overlay precedence and unresolved effective state to the selected scope.
...
Define a versioned rule severity policy and per-instance evaluated severity with contributing factors and rationale. Findings carry separate `match_confidence` and `conclusion_confidence`, each with its basis and limitations. Catalogue manifests pin UUID, document version, OSCAL schema version and content digest; reviewed mapping artifacts reference authoritative control/clause IDs and the pinned catalogue. Implement an internal catalogue adapter that validates these identities and status semantics using the reviewed CISS interpretation. Reject missing, ambiguous or withdrawn IDs/clauses as active mandatory mappings; block a corporate verdict while preserving deterministic findings and mapping errors. Do not silently select replacement controls or infer mappings from catalogue import.
...
Build synthetic benign/vulnerable fixtures for individual skills, MCP packages, a mixed repository, a 20-skill repository, malformed archives, hostile instruction files, installed configurations and captured endpoint responses. Include identical source deployed to different environments, weakened configuration overlays, unavailable effective configuration, withdrawn controls/clauses and mismatched catalogue digests. Cover different severities for two instances of one rule and independent confidence axes. Store expected findings and explicit unknown/error coverage. Synthetic controls are labelled test fixtures; import actual IDs from the authorised CISS source for real assessments and separately review the mappings. Never invent corporate control IDs.

**Done when:** valid examples round-trip through schemas; invalid selectors, arbitrary commands/roles/model profiles, body-supplied ownership and hosted workstation paths are rejected. Every fixture has an expected evidence scope and result, including limitations. Catalogue validation covers the reviewed control/clause lifecycle encoding and invalid-mapping verdict block; internal access or policy decisions still pending remain explicit gates, not synthetic test successes.
...
Discover selected `SKILL.md` packages and their relevant plugin/support files. Identify MCP source targets through explicit package paths and supported manifest/code patterns; preserve ambiguous candidates rather than silently claiming complete discovery. Build one shared file/dependency index and assign reusable per-package subject IDs. Derive separate target-evaluation IDs from the subject, assessment scope and applicable environment/instance identity. Link shared files to all affected targets and freeze the inventory for later resume.

Accept selected deployment configuration and overlays as explicit collector inputs with provenance, hashes and precedence. Bind each deployment/installation evaluation to its environment and instance identity while sharing immutable source parsing. Source-only assessments remain valid and make no deployed-environment claim; unavailable or unresolved configuration makes dependent deployment checks unknown.

**Done when:** a mixed repository is captured once and yields the expected skill/MCP inventory; a 20-skill fixture does not cause 20 clones. Two deployments of identical source retain distinct configuration-dependent evaluations without duplicate parsing; a weakening overlay is assessed at the correct instance and absent configuration cannot become a pass. Symlink/path escapes, concurrent edits and truncated collection produce bounded, explicit results. No target instructions, hooks or scripts activate.

### Step 4 — Deliver the first working local scanner

Implement the required pipeline in `engine/pipeline.py`: collection, inventory, checks, reviewed mappings/policy and reporting. Define a rule registry with applicability, required evidence, versions, deterministic severity policy, confidence bases, limitations, maintained remediation and verification guidance. Evaluate severity and its factors/rationale for each finding instance; calculate `match_confidence` and `conclusion_confidence` separately. Add shared, skill and MCP rule modules; select rules from evidence and permissions, independently of internal/external ownership and local/hosted backend.

Begin with a small reviewed rule set: secret indicators with redaction; dependency inventory/pinning/lockfile/provenance evidence; missing or escaping references; unsafe execution patterns in supported languages; skill capability/instruction indicators; and MCP schema/configuration issues. The engine does not determine CVEs in v1. If approved-scanner result import is enabled, validate supplied result bounds, scanner identity/version, time/freshness and source correspondence, and preserve the external determination's provenance. Heuristic indicators must retain their limitations. Implement `pass`, `fail`, `unknown`, `not_applicable`, `error` and `skipped`; missing implementation evidence cannot become a pass.

Add versioned many-to-many corporate mappings, applicability, exceptions and policy evaluation using the validated internal catalogue adapter. Validate control and clause status at mapping load against the pinned catalogue; preserve historical withdrawn references without counting them as current mandatory obligations. Unmapped findings and mandatory unknowns remain visible. The corporate catalogue is authoritative; external categories are supplementary labels. Block a real corporate compliance verdict when required reviewed catalogue/policy inputs are missing or mandatory mappings are invalid; preserve the report and deterministic findings. Replacements require reviewed mapping changes.

Implement the local storage adapter and typed CLI entry points for assessment, status/report access, cancellation and explicit resume. Write `findings.json`, `coverage.json` and a readable report with evidence references, pinned versions and per-target limits. Use access-controlled run directories, atomic baseline publication, local locking/checkpoints and bounded workers. Bind run manifests, any result-cache keys and resume validation to the captured snapshot, assessment scope, environment/deployment/installation identity, selected configuration digests/precedence, catalogue identity/digest and rule/mapping/policy versions. Resume also verifies current permissions; older interrupted attempts cannot overwrite a resumed attempt's output. Share parsing by immutable content identity without reusing deployment verdicts across contexts. Do not auto-upload results.

**Done when:** a standalone local fast scan completes against one mixed fixture and then the 20-skill batch, with expected findings, mappings and coverage. Identical captured inputs/context/versions yield equivalent deterministic results. Severity remains instance-specific, confidence fields preserve their distinct meanings, and withdrawn/invalid mandatory mappings block the corporate verdict without discarding findings. The engine makes no model call or CVE determination, survives an interrupted run with honest state, rejects changed-context resume/older-attempt publication, and completes with AgentCore/S3/database access denied. The initial technical prototype may use clearly labelled synthetic controls solely as test fixtures.
...
Apply time/page/byte/depth/concurrency budgets, destination and redirect/DNS validation, TLS policy and secret redaction. Record observed transport/authentication metadata, advertised schemas, collection time/location, credential-view identity and partial failures. Bind observations to the selected environment/deployment and configuration evidence; keep source/deployment correspondence explicit and implementation-only checks unknown when evidence is absent.

**Done when:** revision-specific SDK/conformance fixtures cover each explicitly supported MCP protocol revision and transport, including successful, denied, paginated, malformed and hanging discovery; out-of-scope destinations and methods are rejected. Record the negotiated revision and reject unsupported revisions without applying another revision's semantics. Test servers are controlled fixtures started by the test harness, not target processes launched by the scanner. The same captured responses and assessment context produce equivalent checks locally and through the eventual hosted adapter.
...
Add portable bundle export with hashes, target associations, environment/installation identity, selected configuration evidence and precedence, redactions/exclusions, capture provenance and limits. Preserve hashes/references to the accepted redacted evidence without exposing excluded secret values. Filter sensitive configuration before export; omit credential values, OAuth caches, full environments and session transcripts. Capture concurrent updates safely. Export remains explicit, and the collector does not interpret workstation loopback as a hosted endpoint.
...
Implement versioned migrations and the Aurora PostgreSQL storage adapter through RDS Data API. Model ownership, assessments, attempts, source and deployment/installation target identities, finding/coverage relationships, artifact references, upload records and pinned versions. Persist assessment scope, environment reference, selected configuration evidence/digests/precedence and catalogue manifest with the canonical input digest. Add owner-scoped unique idempotency keys/input digests, session bindings, fencing tokens, leases and guarded transitions. Acceptance and initial-attempt creation commit together. Keep SQL parameterised and transactions short; reconcile uncertain commits before repeating writes.
...
Resolve the approved base once, capture/upload its immutable snapshot and invoke the API. Persist the run manifest, assessment scope/environment and selected deployment configuration if supplied, evidence references, idempotency keys and assessment ID across retries and `run_attempt` changes. Default repository-only jobs to `source_only`; do not infer a production deployment from the branch name. Use bounded polling and a documented timeout/cancel policy. Save restricted report references and a summary before applying policy-based workflow exit codes. Use the selected machine API identity and no repository write token.
...
Complete dependency/provenance review, release compatibility checks, IAM/secret denial and rotation tests, logging/redaction, retention/orphan cleanup, Aurora/S3 restore and complete-release rollback. Confirm the scanner-classification, approval and recurring configuration-review decisions made in steps 1–2 remain satisfied; do not discover this requirement for the first time at release. Publish operating guidance for partial/interrupted reports, explicit resume, source-only versus deployment conclusions, mandatory unknowns, withdrawn mappings, unsupported targets and stale PRs. Promote the same tested engine/plugin/rule/configuration combination from development to production.
...
| --- | --- | --- |
| Authorised internal CISS location/access, OSCAL encoding and control/clause status interpretation | Catalogue adapter validation in step 2; real corporate verdicts in step 4 | Schemas and clearly labelled synthetic test fixtures |
| Corporate scanning-tool classification, applicable approval and configuration-review requirements | Decisions in steps 1–2; applicable approval before real scanning/pilot use | Isolated contract and synthetic fixture development |
| Reviewed mappings, per-instance severity policy and mandatory-unknown policy | Real corporate policy verdicts in step 4; hosted pilot | Rule mechanics and synthetic tests; catalogue import does not supply these decisions |
| Environment/deployment identifiers and selected configuration evidence/precedence | Deployment-scope acceptance in steps 3–6 | Source-only assessment and shared parsing |
| Approved-scanner result format/provenance and authority, if vulnerability import is required | Enabling bounded supplied-result ingestion in step 4 | Dependency inventory/pinning/lockfile/provenance evidence without engine CVE determination |
...
| --- | --- | --- |
| 1 | Package skeleton, CI, schema models, internal catalogue adapter, rule/storage interfaces and fixture conventions | Installable wheel; catalogue identity/status validation; environment/severity/confidence contracts; invalid-input fixtures; explicit synthetic-control labelling; scanner-classification decision or blocker |
| 2 | Bounded local snapshot/configuration collector, archive handling, mixed target inventory and shared file index | Stable inventory/hashes; distinct deployment evaluations; overlay and concurrent-edit cases; no target activation |
| 3 | First reviewed rules, mapping/policy adapter, JSON/readable reporting and local fast CLI | Known findings/unknowns on mixed and 20-skill fixtures; instance severity/confidence and invalid-mapping gates; no model/cloud calls or engine CVE determination; reproducible baseline and context-bound interruption records |
